...
xpl = 32
#position of y-z plane

CIJ = ['CXX','CXY','CXZ','CYY','CYZ','CZZ']
cij = ['cxx','cxy','cxz','cyy','cyz','czz']
dir_name = 'infertanh'
out_name = 'unet_x{0}stuicij'.format(xpl)
case_name = 'unet_x{0}stui{1}'.format(xpl,cij[index])
...
#    with open(os.path.join(dirpath,'data/383+20000per40/CHAN%03dDU.DAT') % i,'r') as fd1:
    with open('./data/383+20000per40/CHAN%03dDU.DAT' % i,'r') as fd1:
        chunk1 = np.fromfile(fd1, dtype=dtu, count=1)
#    with open(os.path.join(dirpath,'data/383+20000per40/CHAN%03dDV.DAT') % i,'r') as fd2:
    with open('./data/383+20000per40/CHAN%03dDV.DAT' % i,'r') as fd2:
        chunk2 = np.fromfile(fd2, dtype=dtv, count=1)
#    with open(os.path.join(dirpath,'data/383+20000per40/CHAN%03dDW.DAT') % i,'r') as fd3:
    with open('./data/383+20000per40/CHAN%03dDW.DAT' % i,'r') as fd3:
        chunk3 = np.fromfile(fd3, dtype=dtw, count=1)
#    with open(os.path.join(dirpath,'data/383+20000per40/CHAN%03dD{0}.DAT'.format(CIJ[index])) % i,'r') as fd11:
    ...
    for x in range(1,65):
        Ui = np.concatenate((U1[x+4,3:99,5:69][:,:,None],V1[x+4,3:99,5:69][:,:,None],W1[x+4,3:99,5:69][:,:,None]),axis=2)
#    Ui = np.transpose(Ui, (0,2,1))
        U.append(Ui)
        CXX.append(CXX1[x+2,1:97,3:67][:,:,None])
    print i
