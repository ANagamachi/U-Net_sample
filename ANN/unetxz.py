...
#        dec2 = concatenate([dec2, enc6], axis=self.CONCATENATE_AXIS)
        ...
        dec5 = concatenate([dec5, enc1], axis=self.CONCATENATE_AXIS)

        # (64 x 64 x output_channel_count)
        dec6 = Activation(activation='relu')(dec5)
        dec6 = Conv2DTranspose(output_channel_count, self.DECONV_FILTER_SIZE, strides=self.DECONV_STRIDE)(dec6)
        dec6 = Activation(activation='linear')(dec6)

        self.UNET = Model(input=inputs, output=dec6)
